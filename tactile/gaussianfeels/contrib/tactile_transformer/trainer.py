...
class Trainer(object):
    def __init__(self, config):
        ...
        self.device = get_device(self.config["General"]["device"])
        ...
        # Load pre-trained model if available (optional for testing)
        path_model = os.path.join(config["General"]["path_model"], "FocusOnDepth.p")
        if os.path.exists(path_model):
            print(f"Loading pre-trained model from {path_model}")
