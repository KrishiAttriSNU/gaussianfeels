...
def sample_along_rays(
    ...
    with torch.set_grad_enabled(grad):
        ...
        # dirs_C are vectors in local camera coordinates, transformed to world coordinates below
        origins, dirs_W = geometry.origin_dirs_W(T_WC, dirs_C)
        ...
        valid_ray = None
        if method2:
            # Using ray_box_intersection from shared geometry
            min_depth_box, max_depth_box = geometry.ray_box_intersection(
                origins,
                dirs_W,
                box_extents,
                box_transform,
