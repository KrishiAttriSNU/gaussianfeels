...
from .frame_data import FrameData, pose_from_config, create_identity_pose
...
import git
# STRICT: Must be run from within a git repository
try:
    root = git.Repo(".", search_parent_directories=True).working_tree_dir
except git.exc.InvalidGitRepositoryError:
    raise EnvironmentError("Code must be run from within a git repository - no fallback to current directory allowed")
        ...
        if "realsense" in self.sensor_name:
            ...
            if self.data_root and self.camera_name:
                ...
                if os.path.exists(rgb_path) and os.path.exists(depth_path):
                    ...
                    # Load pose data from data.pkl (this would need proper implementation)
                    raise NotImplementedError("Camera pose loading from data.pkl not yet implemented - no default identity fallback allowed")
