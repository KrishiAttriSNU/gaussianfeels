...
class TactileDepth:
    def __init__(self, depth_mode, real=False, device="cuda"):
        ...
        if depth_mode == "gt":
            self.model = None
            return
        elif depth_mode in ["vit", "touchvit"] or (hasattr(cfg, 'mode') and cfg.mode == "vit"):
            # print("Loading ViT depth model----")
            self.model = TouchVIT(cfg=cfg)
        else:
            # STRICT: No fallback modes allowed - must be explicitly configured
            raise ValueError(f"Mode '{depth_mode}' not implemented. Only 'gt' and 'vit'/'touchvit' modes are supported - no fallback behavior allowed")
        # print("done")
        self.device = device
