...
import numpy as np
from ..utils.spatial_hash import CUDASpatialHash, SpatialHashConfig
...
class DensifyPruneConfig:
    ...
    densify_grad_threshold: float = 0.0002  # Stricter threshold for higher quality
...
class DensifyPruneManager:
    ...
    def _should_densify(self) -> bool:
        """Check if densification should be performed - NO DELAYS"""
        return True  # Always densify when called, no artificial delays
    
    def _should_prune(self) -> bool:
        """Check if pruning should be performed - NO DELAYS"""
        return True  # Always prune when called, no artificial delays
    ...
    def _accumulate_gradients(self, gaussian_map, optimizer_state: Dict):
        ...
        if 'position_gradients' not in self.gradient_accum:
            ...
            alpha = 0.9
            self.gradient_accum['position_gradients'] = (
                alpha * self.gradient_accum['position_gradients'] + 
                (1 - alpha) * grad_magnitudes
            )
    ...
    def _identify_densify_candidates(self, gaussian_map) -> torch.Tensor:
        ...
        high_grad_mask = grad_magnitudes > grad_threshold
    ...
    def _split_gaussians(self, gaussian_map, split_indices: torch.Tensor) -> int:
        ...
        # Reduce scale of the split axis
        new_scales = scales.clone()
        new_scales[torch.arange(len(split_indices)), largest_scale_idx] -= math.log(2.0)
        ...
        # Reduce opacity slightly for new Gaussians
        reduced_opacity = opacity * 0.8
        all_new_opacity = torch.cat([reduced_opacity, reduced_opacity], dim=0)
    ...
    def _prune_gaussians(self, gaussian_map) -> int:
        ...
        # 4. Spatial redundancy pruning (if gaussian_map exposes neighbor queries)
        try:
            redundant_mask = self._identify_spatially_redundant_gaussians(gaussian_map)
            prune_mask |= redundant_mask
        except (RuntimeError, ValueError, AttributeError) as e:
            raise RuntimeError(f"Spatial redundancy pruning failed: {e}") from e
    ...
    def _identify_spatially_redundant_gaussians(self, gaussian_map) -> torch.Tensor:
        """Identify spatially redundant Gaussians using neighbor queries when available."""
        positions = getattr(gaussian_map, 'positions', None)
        if positions is None or positions.numel() == 0:
            return torch.zeros(0, dtype=torch.bool)
        device = positions.device
        redundant_mask = torch.zeros(len(positions), dtype=torch.bool, device=device)

        # Prefer gaussian_map.query_neighbors if provided
        query_fn = getattr(gaussian_map, 'query_neighbors', None)
        if callable(query_fn):
            for i in range(len(positions)):
                pos = positions[i]
                try:
                    nbr_idx = query_fn(pos, radius=0.01)
                except (RuntimeError, ValueError, TypeError) as e:
                    raise RuntimeError(f"Neighbor query failed for position {pos}: {e}") from e
                if nbr_idx is None or len(nbr_idx) == 0:
                    continue
                # Too many close neighbors; mark lower-opacity ones redundant
                opacity = getattr(gaussian_map, 'opacity', None)
                if opacity is None or opacity.numel() == 0:
                    continue
                neighbor_opacities = opacity[nbr_idx].squeeze()
                best = int(nbr_idx[torch.argmax(neighbor_opacities)])
                if i != best and len(nbr_idx) > 6:
                    redundant_mask[i] = True
            return redundant_mask

        raise RuntimeError("Neighbor query function 'query_neighbors' is required for redundancy pruning")
