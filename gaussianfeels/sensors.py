...
class TactileSensor(BaseSensor):
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            # Mock sensor removed for strict fail-fast operation
            else:
                # No default implementation - require user to provide one
                raise RuntimeError(
                    f"No tactile hardware backend for sensor type '{sensor_type}' on {self.config.name}. "
                    "Either provide a concrete implementation by setting _custom_sensor_client "
                    "or use a supported sensor type: digit, gelsight, tacto"
                )
