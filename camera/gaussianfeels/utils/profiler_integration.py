...
if __name__ == "__main__":
    ...
    def mock_gaussian_operation(n_gaussians: int = 10000):
        """Example Gaussian operation for testing"""
        # Simulate Gaussian parameter computation
        positions = torch.randn(n_gaussians, 3)
        scales = torch.abs(torch.randn(n_gaussians, 3))
        rotations = torch.randn(n_gaussians, 4)  
        colors = torch.rand(n_gaussians, 3)
        
        # Simulate some computation
        distances = torch.cdist(positions, positions)
        weights = torch.exp(-distances / scales.mean())
        result = torch.sum(weights * colors.unsqueeze(1), dim=2)
    ...
    def mock_optimizer(params, target_data, iteration=0):
        """Example optimizer for testing"""
        return mock_gaussian_operation(1000 + iteration * 10)
    
    # Run tests
    print("\n1. Testing function profiling...")
    result = mock_gaussian_operation(5000)
    print(f"   Mock operation completed with loss: {result['loss']:.6f}")
    
    print("\n2. Testing optimization profiling...")
    optimization_results = profiler.profile_gaussian_optimization(
        mock_optimizer,
        torch.randn(1000),  # Mock gaussian params
        {'target_points': torch.randn(1000, 3)},  # Mock target data
        n_iterations=20,
        profile_every_n=5
    )
