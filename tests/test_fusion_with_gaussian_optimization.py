...
class FusionGaussianOptimizationTest:
    ...
    def __init__(self):
        ...
        self.data_path = str(repo_root / 'data/feelsight/010_potted_meat_can/00')
        # Object-specific output directory (from data path)
        object_name = Path(self.data_path).parts[-2] if len(Path(self.data_path).parts) >= 2 else Path(self.data_path).name
    ...
    def test_01_run_complete_fusion_pipeline(self, max_frames=10):
        ...
        try:
            ...
            from camera.gaussianfeels.io.rgbd_dataset import RGBDDataset
            from camera.gaussianfeels.io.depth_filters import filter_depth_percentile, undistort_depth
            from camera.gaussianfeels.io.image_transforms import DepthTransform, BGRtoRGB, DepthScale
    ...
    def test_02_gaussian_field_initialization(self):
        ...
        try:
            ...
            from camera.gaussianfeels.core.gaussian_field import ObjectGaussianMap, GaussianConfig
    ...
    def test_03_pose_optimization(self):
        ...
        try:
            ...
            # NO GAUSSIAN REDUCTION - Use all Gaussians, fail fast if memory issues
    ...
    def test_04_surface_constraints(self):
        ...
        try:
            ...
            from camera.gaussianfeels.loss.volumetric_loss import VolumetricLossFunction, MultiModalGaussianLoss
            from camera.gaussianfeels.render.rasterizer import render_rgbd
    ...
    def test_05_densification_pruning(self):
        ...
        try:
            ...
            from camera.gaussianfeels.core.densify_prune import DensifyPruneManager, DensifyPruneConfig
            
            gaussian_field = self.optimization_results['gaussian_field']
            initial_gaussians = gaussian_field.num_active_gaussians
            
            # Create densify/prune configuration
            densify_config = DensifyPruneConfig(
                densify_grad_threshold=0.002,  # Very low threshold for aggressive densification
                densify_interval=10,  # Much more frequent densification
                prune_opacity_threshold=0.001,  # Keep almost all Gaussians
                prune_interval=50,  # Less frequent pruning
                max_gaussians=initial_gaussians + 20000,  # Allow major growth
                enable_adaptive_thresholds=True
            )
            
            print(f"📈 Densify/Prune config:")
            print(f"   📊 Initial Gaussians: {initial_gaussians:,}")
            print(f"   📏 Grad threshold: {densify_config.densify_grad_threshold}")
            print(f"   🔄 Max Gaussians: {densify_config.max_gaussians:,}")
            ...
            # Short real loop with memory management
            n_steps = 5
            ...
            for step in range(n_steps):
                ...
                print(f"  Step {step+1}: {current_gaussians:,} Gaussians (+{densify_result.get('densified', 0)}, -{densify_result.get('pruned', 0)})")
            ...
            print(f"   📈 Processing steps: {n_steps}")
...
"""
    ...
    def test_07_learning_based_pose_optimization(self):
        ...
        try:
            ...
            from camera.gaussianfeels.io.rgbd_dataset import RGBDDataset
            from camera.gaussianfeels.core.gaussian_pose_optimizer import SE3TangentSpace
    ...
    def test_09_performance_diagnostics_integration(self):
        ...
        try:
            ...
            from camera.gaussianfeels.memory.memory_monitor import MemoryMonitor, create_default_alert_handler
            from camera.gaussianfeels.utils.performance_diagnostics import PerformanceDiagnostics, get_global_diagnostics
            from camera.gaussianfeels.instrumentation.live_counters import LiveCounters, live_counters
            ...
            try:
                ...
                cx_s = small_w / 2.0
                cy_s = small_h / 2.0
                from camera.gaussianfeels.render.rasterizer import CameraParams, RenderConfig
