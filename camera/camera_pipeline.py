...
from camera.gaussianfeels.utils.pose_transforms import transform_points_to_object_frame, select_optimal_keyframes
from camera.gaussianfeels.io.segmentation import (
    SegmentationProcessor,
    FeelsightSegmentationLoader,
    FeelsightRealSegmentationLoader,
)
from camera.gaussianfeels.io.depth_filters import undistort_depth, filter_depth_percentile
    ...
    def process_single_frame(self, frame_idx: int, camera_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if 'intrinsics_matrix' not in cam_info:
                    raise KeyError(f"Camera '{cam_name}' missing required 'intrinsics_matrix'")
                K = np.array(cam_info['intrinsics_matrix'], dtype=np.float32)
                if 'distortion_coeffs' not in cam_info:
                    raise KeyError(f"Camera '{cam_name}' missing required 'distortion_coeffs'")
            ...
            # Apply outlier rejection + segmentation mask
            depth_filtered = self._neuralfeels_filter_depth(depth, outlier_thresh=7.0, cutoff_depth=3.5)
