...
class FeelsightRealSegmentationLoader:
            ...
            try:
                ...
                if depth_npz.exists() and mask.sum() > 0:
                    ...
                    if obj_depth_vals.size > 0:
                        ...
                        sigma = 0.04  # 4cm band
                        zmin, zmax = z_med - 2 * sigma, z_med + 2 * sigma
                        ...
                        if num_labels > 1 and mask_pixels is not None and len(mask_pixels) > 0:
                            ...
                            final_mask = gated
                        # Require sufficient mask coverage - no fallbacks
                        if final_mask.sum() < max(500, int(0.1 * mask.sum())):
    ...
    def _get_visible_sensors(self, digit_poses, realsense_pose):
        """Visibility computation requires proper depth rendering - no fallbacks"""
        raise NotImplementedError(
            "Visibility fallback removed. Implement proper visibility computation "
            "using rendered depth maps and occlusion testing."
        )
