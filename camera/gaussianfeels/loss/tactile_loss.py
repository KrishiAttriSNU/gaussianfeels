...
class TactileSurfaceLoss(nn.Module):
    ...
    def _compute_depth_supervision_loss(self, tactile_points: torch.Tensor,
                                      rendered_depth: torch.Tensor) -> torch.Tensor:
        """
        Additional supervision from rendered depth if available.
        """
        # This would need camera parameters and projection
        # Simplified implementation assuming aligned depth map
        return torch.tensor(0.0, device=tactile_points.device)
