...
class TactileDataset(Dataset):
    def __init__(
        ...
        """DIGIT dataset loader for tactile dataset"""
    ...
    def get_avg_contact_area(self):
        """
        Returns the average contact area of the tactile dataset
        """
        contact_area = 0.0
        for i in range(len(self)):
            _, depth = self.__getitem__(i)
            if depth is not None:
                mask = depth > 0  # Contact mask
                contact_area += mask.sum() / mask.size
        contact_area /= len(self)
        return contact_area


class VisionDataset(torch.utils.data.Dataset):
    """RealSense data loader for tactile dataset"""
    ...
    def __init__(
        ...
        if sim_noise_iters > 0:
            ...
            from .redwood_depth_noise_model import add_noise
            ...
            for i, depth in enumerate(tqdm(self.depth_data)):
                depth = add_noise(-depth, self.dist_model, sim_noise_iters)
