...
from tactile.gaussianfeels import geometry
from tactile.gaussianfeels.modules import sample

# Viz import commented out - not needed for core functionality
# from tactile_lib import viz
...
def test_inside_frustum(T_WC, depth):
    ...
    # show camera
    scene = trimesh.Scene()
    # viz.draw.draw_cams(1, T_WC, scene)  # Commented out - viz module not available

    # show random point cloud
    points = np.random.normal(0.0, 2.0, [1000, 3])
    ...
    z_vals = sample.stratified_sample(0.2, max_depth, n_rays, "cpu", n_stratified_samples=12)
    ...
    normal_lines = np.concatenate((starts[:, None, :], normal_ends[:, None, :]), axis=1)
